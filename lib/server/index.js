'use strict';

const Restify = require('restify');
const Mongoose = require('mongoose');
const Errors = require('storj-service-error-types');
const AuditModel = require('../fullauditmodel');
const Log = require('../logger');
const FS = require('fs');
const Assert = require('assert');

function AuditServer(opts) {
  this._options = opts;

  var serverConfig = {
    name: 'storj-service-auditor'
  };

  this._mongoConnection = Mongoose.createConnection(
    this._options.mongo.uri,
    this._options.mongo.options
  );

  this._auditModel = new AuditModel(this._mongoConnection);

  if(this._options.server.cert || this._options.server.key || this._options.server.ca || this._options.server.passphrase) {
    serverConfig.httpsServerOptions = (serverConfig.httpsServerOptions) ? serverConfig.httpsServerOptions : {};

    if(this._options.server.cert) {
      serverConfig.httpsServerOptions.cert = FS.readFileSync(this._options.server.cert);
    }

    if(this._options.server.key) {
      serverConfig.httpsServerOptions.key = FS.readFileSync(this._options.server.key);
    }

    if(this._options.server.ca) {
      serverConfig.httpsServerOptions.ca = this._options.server.ca.split(' ').map(function(ca) {
        return FS.readFileSync(ca);
      });
    }

    if(this._options.server.passphrase) {
      serverConfig.httpsServerOptions.passphrase = this._options.server.passphrase;
    }
  }

  Log.info('starting server...');
  this.server = Restify.createServer(serverConfig);

  this.server.use(Restify.bodyParser({
    maxBodySize: 0,
    mapParams: true,
    mapFiles: false,
    overrideParams: true
  }));

  this.server.listen(
    this._options.server.port,
    this._options.server.host
  );

  Log.info('listening for audits on: ' + this._options.server.host + ':' + this._options.server.port);

  this.server.post('/audit-schedule',
    (req, res, next) => {
      req.accepts('application/json');
      try {
        Assert(req.params.farmer_id);
        Assert(req.params.data_hash);
        Assert(req.params.root);
        Assert(req.params.depth);
        Assert(req.params.challenges && req.params.challenges instanceof Array);
        Assert(req.params.start);
        Assert(req.params.end);
      } catch(err) {
        return next(err);
      }

      return next();
    },

    (req, res, next) => {
      this._auditModel.scheduleFullAudits(
        {
          farmer: req.params.farmer_id,
          hash: req.params.data_hash,
          root: req.params.root,
          depth: req.params.depth,
          challenges: req.params.challenges,
          start: req.params.start,
          end: req.params.end
        },
        null,
        (err) => {
          if (err) {
            return next(Errors.InternalError);
          }

          res.send(201);
          return next();
        }
      );
    }
  );
}

module.exports = AuditServer;
